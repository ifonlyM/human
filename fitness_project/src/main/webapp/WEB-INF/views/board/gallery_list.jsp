...
<div class="justify-content-end ">
	<form class="form-inline" method="get" action="/board/gallery_list">
			...
			<c:if test="${not empty board}">
				<div class="card-body">
					<div class="uploadResult">
						<ul class="list-group">
							<li style="list-style: none"><img alt="썸네일" src="/display?fileName=${board.attachs[0].path}/s_${board.attachs[0].uuid}.${board.attachs[0].ext}" style="width: 200px"> </li>
						</ul>
					</div>	
				</div>
    			<div class="text-nowrap text-truncate text-center" style="width: 250px">
    				<a href="get${page.cri.params}&bno=${board.bno }&category=${category}"><c:out value="${board.title }" /><b>[${board.replyCnt}]</b></a>
    			</div>
			</c:if>
			</div>
			   	...
			   	<div class="st-pagination">
					<ul class="pagination justify-content-center">
						<li class="page-item previous ${page.prev ? '' : 'disabled'}" id="dataTable_previous">
							<a class="page-link bg-dark text-white" href="gallery_list?pageNum=${page.startPage-1}&amount=${page.cri.amount}">Previous</a>
						</li>
						
						<c:forEach begin="${page.startPage }" end="${page.endPage }" var="p" >
						   <li class="page-item ${p == page.cri.pageNum ? 'active' : ''}">
						   	<a class="page-link" href="gallery_list?pageNum=${p}&amount=${page.cri.amount}">${p}</a>
						   </li>
						</c:forEach>
						
						<li class="page-item ${p == page.cri.pageNum ? 'active' : '' }" id="dataTable_next">
							<a class="page-link bg-dark text-white" href="gallery_list?pageNum=${page.endPage+1}&amount=${page.cri.amount}">Next</a>
						</li>
					</ul>
				</div>	
			   </div>
